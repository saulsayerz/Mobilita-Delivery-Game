...
float determinant(Matrix m)
{
    ...
    { //Ini buat 2x2 keatas, aslinya 2x2 sama 3x3 bisa cara biasa tapi gapapadeh ikut direkursifin
        for (i = 0; i < ROWS(m); i++)
        { // ini untuk baris paling atas (pertama) kolom ke i nanti dikaliin minornya
            for (j = 1; j < ROWS(m); j++)
            { // ini startnya dari baris kedua karena kofaktornya dari situ
                for (k = 0; k < COLS(m); k++)
                {
                    if (k < i)
                    {
